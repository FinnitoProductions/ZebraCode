...
/*
* A template which, in slot "a" stores the type of fact to be asserted (either house color, nationality, pet, drink, 
* or cigarette type), in slot "v" stores the value corresponding to that fact type (like a specific house color (ex: red), 
* nationality (ex: englishman), pet (ex: dog), drink (ex: milk), or cigarette type (ex: old-golds)), and in slot "h"
* stores the house position of the person with this characteristic, which ranges from 1-5 in this problem. The house
...
* representing each piece of information in the solution set.
*
* Will only be triggered if all the facts required to satisfy the problem have been asserted; a combination of 
* the facts asserted must collectively meet each of the requirements as dictated by the problem statement.
*/
(defrule find-solution
  ...
  */
  ...
  /*
  * The dog-owner lives at some house location (?p1) which is at the same location as the Spaniard's house (?n2) because
  * the Spaniard owns the dog.
  */
  ...
  * nor the red house (?c1). Furthermore, the location of the green house is one to the right of (one index greater than) 
  * the location of the ivory house (?c2) because the ivory house is one to the left of the green house.
  */
  ...
  * and is equal to 3 because the milk-drinker lives in the middle house position.
  */
  ...
  * of the Chesterfield-smoker (?s2) or the Old Golds-smoker (?s1).
  */
  ...
  * Japanese (?n5) because the Japanese man smokes Parliaments.
  */
  ...
  */
  ...
  /*
  * The Kools-smoker lives at some house location (?s5) not the same as that of the Parliaments smoker (?s4), 
  * the Lucky Strike-smoker (?s3), the Chesterfield-smoker (?s2), nor the Old Golds smoker (?s1).
  * Furthermore, because the Kools smoker's house is next door to the horse-owner's house (?p4), the 
  ...
  * the ivory house (?c2), nor the red house (?c1). Also, the blue house is either one to the left (one index less than)
  * or one to to the right of (one index greater than) the Norwegian's house (?n4) because the Norwegian lives next to the blue house.
  */
  ...
  */
  ...
  => 
  /*
  * Asserts the solution set to the problem. Because this call only triggers when all of the above conditions have
  * been met, the value stored in each of the variables will correspond to the relevant value from the solution set.
  */
...
/*
* Prints out the solution to the Zebra problem by storing all the solution facts asserted by the
* find-solution rule and retracting each one, then printing the solution set for each house location's
* corresponding nationality, house color, pet, preferred drink, and cigarette type.
*/
(defrule print-solution
  /*
  * Stores each of the solution facts into variables to be retracted. Stores the information within each of 
  * the solution facts in additional variables so that the solution can be printed. 
  * Note that the following variables do not represent the same information as those stored in the 
  * previous rule (find-solution), but instead are numbered by house location.
  */
  ...
  =>
  /*
  * Retracts all the solution facts previously asserted - it is unclear why this is necessary.
  */
...
* each piece of information required to solve the problem. After printing the problem statement, asserts all possible
* values for house color, nationality, pet, drink, and cigarette type, each of which will trigger the generate-combinations
* rule.
*/ 
(defrule startup
   =>
...
* value of that fact type has been asserted, generates all five combinations for that fact by pairing each piece
* of information with each of the five house locations using the above-defined avh template.
*
* For each of the values pairing a given fact type with a fact value (as asserted by the above startup rule), 
* this rule will ensure that each of those values has been paired with all possible house positions to guarantee
* that every candidate for each solution fact has been asserted. Asserting all possibilities will guarantee that the 
* find-solution rule will be triggered if the problem has a solution. 
*/
(defrule generate-combinations
   ?f <- (value ?s ?e) ; stores the fact into a value so it can be later retracted
   =>
   (retract ?f) ; retracts the fact after its information has been stored in variables - unclear why the retract is necessary

   /* 
   * Asserts the piece of information which triggered the rule with all possible house locations to ultimately 
   * generate all possibilities for each person.
   */
...
/* 
* Prevents a call to "reset" from clearing out global variables to ensure the *time* variable is not 
* reinitialized when the rule engine is reset.
*/
...
(deffunction run-n-times (?n)
  (while (> ?n 0) do ; iterates a total of ?n times, resetting and then running the rule engine for each iteration
         (reset)
         (run)
         (bind ?n (- ?n 1)))) 

(run-n-times 1) ; runs the program once (when the code is first executed) to solve the problem

/*
* Prints how long the rule engine took to run by subtracting the time before the rule engine was run from the current time.
*/
